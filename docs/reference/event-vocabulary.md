...
| `STOP_ORDER_ACCEPTED` | [722](https://github.com/GabrieleDiCorato/abides-ng/blob/main/abides-markets/abides_markets/agents/exchange_agent.py#L722) | `O` (gated on `exchange_log_orders`) | `str` (`str(order)`) | external | `dict` (`order.to_dict()`) | `[REVIEW]` change to dict for parity with other accept events | The only fixed-name event the exchange emits via this path; payload shape disagrees with the rest of the lifecycle. |
