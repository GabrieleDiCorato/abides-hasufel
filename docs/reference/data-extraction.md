...
`LiquidityMetrics.vwap_cents` is computed from order-book execution history — an integer-cents volume-weighted average price across all trades in the session.
...
See [metrics-api.md](metrics-api.md) for the full metric
