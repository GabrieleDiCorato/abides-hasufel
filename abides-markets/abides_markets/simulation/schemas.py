...
_REJECTED_ORDER_EVENT_TYPES = frozenset({"ORDER_REJECTED"})
...
class OrderLogsSchema(RawLogsSchema):
    ...
    columns that are guaranteed on every order-related log entry.  ``ORDER_REJECTED``
    rows are excluded from this DataFrame because they lack ``symbol``,
    ``quantity``, and ``side`` — use
    :meth:`~abides_markets.simulation.SimulationResult.rejected_order_logs` for
    those.  Extra per-order-type columns (``is_hidden``, ``time_in_force``, etc.)
    remain accessible but are not declared here.
    """
...
class RejectedOrderLogsSchema(RawLogsSchema):
    ...
    reason: Series[str] = pa.Field(
        description="Rejection reason (e.g. 'INVALID_PRICE', 'UNKNOWN_SYMBOL')."
    )
