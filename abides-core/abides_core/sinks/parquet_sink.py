...
from ..telemetry.event_payloads import EVENT_TYPE_SCHEMA, GENERIC, PayloadSchema
...
class ParquetSink:
    ...
    Example::
        ...
        from abides_core.sinks.parquet_sink import ParquetSink, read_parquet_logs
    ...
    def on_metric(self, t: tuple) -> None:
        ...
        if bucket is None:
            bucket = _Bucket(self._metric_columns, _build_metric_schema(self._pa, key))
            self._metric_buckets[key] = bucket
        if bucket.next_seq_lo is None:
            bucket.next_seq_lo = seq
        # value is documented as float; coerce to float for Arrow stability.
        bucket.append((agent_id, agent_type, sim_time_ns, float(value), seq))
    ...
    def on_book_snapshot(self, t: tuple) -> None:
        ...
        if bucket.next_seq_lo is None:
            bucket.next_seq_lo = seq
        bucket.append((sim_time_ns, _pickle(bids), _pickle(asks), int(depth), seq))
...
def unpickle_payloads(df: pd.DataFrame, column: str = "payload") -> pd.DataFrame:
    ...
    out[column] = out[column].map(
        lambda b: pickle.loads(b) if b is not None else None
    )  # noqa: S301
    return out
...
def _validate_file_metadata(md: dict[bytes, bytes] | None, path: Path) -> None:
